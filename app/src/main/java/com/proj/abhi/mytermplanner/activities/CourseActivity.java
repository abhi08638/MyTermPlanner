...
public class CourseActivity extends GenericActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //init cursor loaders
        navBundle.putInt(Constants.CURSOR_LOADER_ID,Constants.CursorLoaderIds.COURSE_ID);
        navBundle.putString(Constants.Sql.COL1,Constants.Course.COURSE_TITLE);
        navBundle.putString(Constants.Sql.WHERE,Constants.Ids.TERM_ID+"="+termId);
        getLoaderManager().initLoader(0, navBundle, this);
        handleRotation(savedInstanceState);
        //init tabs
        initViewPager();

        navMenuPojo=new NavMenuPojo(Constants.MenuGroups.COURSE_GROUP,getString(R.string.courses),
                getString(R.string.create_course),Constants.Course.COURSE_TITLE);
    }
    ...
    protected void save() throws Exception {
        ...
        currentUri=courseDetailFragment.save();
        refreshMenu();
    }
    ...
    public void initActivityFromFragment(Bundle b){
        termId=b.getInt(Constants.Ids.TERM_ID);
        navBundle.putString(Constants.Sql.WHERE,Constants.Ids.TERM_ID+"="+termId);
        refreshMenu();
    }
}
